...
def pyzip(file):
    ...
    iivname,ext = os.path.splitext(filename)
    if ext==".lnk":
...
def openzip(file,chef):
    ...
    if ext!=".dec":
        print(' 対応していないファイル形式です。')import xcode
import pyminizip
import os
import time,sys
import warnings
...
while True:
    ...
    if c=="N" or c=="n":
        ...
        print('')#g
        print(ext);knh
        input(' メニュー画面に戻るにはエンターjキーを押してください')w
        return
    basename_without_ext = os.path.splitext(os.path.basename(filename))[0]
    try:
        print('')
